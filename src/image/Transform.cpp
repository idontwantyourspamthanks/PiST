...
QVector<ImagePhase> insertFramesIntoPhases(const QVector<ImagePhase> &phases, int index, int count)
{
    if (count <= 0)
        return phases;
    QVector<ImagePhase> next;
    next.reserve(phases.size());
    for (ImagePhase p : phases) {
        const int start = p.start >= index ? p.start + count : p.start;
        int end = p.end >= index ? p.end + count : p.end;
        if (p.end == index - 1 && p.start <= p.end)
            end = p.end + count;
        p.start = start;
        p.end = end;
        next.append(p);
...
{
    QVector<ImagePhase> next;
    for (const ImagePhase &p : phases) {
        if (p.start == index && p.end == index)
            continue;
        ImagePhase q = p;
        q.start = p.start > index ? p.start - 1 : p.start;
        q.end = p.end >= index ? p.end - 1 : p.end;
        if (q.start > q.end)
            continue;
        next.append(q);
    }
...
QVector<ImagePhase> clampPhases(const QVector<ImagePhase> &phases, int frameCount)
{
    ...
    for (ImagePhase p : phases) {
        if (p.start > last)
            continue;
        p.start = std::max(0, p.start);
        p.end = std::max(p.start, std::min(p.end, last));
        next.append(p);
    }
    return next;
}
...
{
    return frameIndex >= phase.start && frameIndex <= phase.end;
}
