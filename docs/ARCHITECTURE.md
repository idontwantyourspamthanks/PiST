...
| `src/control/` | `RemoteControl` — the localhost TCP line protocol that drives the IDE from a script or an AI agent, with `watch`/`unwatch` event subscriptions — and `mcp/`, the `pist-mcp` stdio MCP shim that bridges MCP clients to it. |
...
- **`editor/CodeEditor.{h,cpp}`** — `QPlainTextEdit` subclass with a `LineNumberArea` gutter.
  ...
  line, error underlines and the find hits via extra selections. Its find bar (a child widget along
  the bottom edge, with the viewport margin to match) searches as you type, wraps, counts the hits,
  and replaces one or all — replace-all back to front inside one edit block, so it undoes in one
