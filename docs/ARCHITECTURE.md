...
| `src/image/` | Sprite document and ST graphics: `ImageDocument` (`.pim` JSON, layers, frames, phases, sheet regions — `cropped()` + `StFormats::exportRegion()` for per-sprite export), `Palette` (STfm 512 / STe 4096 cubes and colour words), `Tools` (brush/line/rect/fill), `Transform` (flip, rotate, onion/phase math), `StFormats` (PI1, NEO, IFF, STOS MBK, PNG, assembler include, plus `spriteSafeDocument()` — the palette shift that keeps colour 0 as background so an export re-imports losslessly). |
